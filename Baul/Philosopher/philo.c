...
int	create_philos(pthread_t *philos_t, t_philo *philo_s, pthread_mutex_t *mutex,t_param *param)
{
	int				i;
...
int	createParams(pthread_t **philos_t, t_philo **philo_s, pthread_mutex_t **mutex, t_param	*param)
{
	int				i;
...
int	main(int argc, char **argv)
{
	...
	createParams(&philos_t, &philo_s, &mutex, &param);
	create_philos(philos_t, philo_s, mutex, &param);
	free(mutex);
	free(philos_t);
	free(philo_s);
	printf("Fin\n");
